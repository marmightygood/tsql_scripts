...
BEGIN
	...
	select 
		@Values = 
		CASE
			WHEN types.name IN ('VARCHAR', 'NVARCHAR') THEN '''' + CAST(SUBSTRING(@LoremIpsum, CAST(RAND() * columns.max_length / 4  AS INTEGER), CAST(RAND() * columns.max_length / 2 AS INTEGER)) AS VARCHAR(MAX)) + ''''
			WHEN types.name = 'INTEGER' THEN CAST(CAST(RAND() * 1000 AS INTEGER) AS VARCHAR(MAX))
			WHEN types.name = 'UNIQUEIDENTIFIER' THEN '''' +  CAST(NEWID() AS VARCHAR(1024)) + ''''
			WHEN Types.name IN ('DATE', 'DATETIME', 'DATETIME2') THEN '''' + CAST(GETDATE() AS VARCHAR(20)) + ''''
			ELSE ''''''
		END
		+ N',' + @Values
	from
